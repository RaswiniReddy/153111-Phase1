...
public class WalletServiceImpl implements WalletService {
	...
	public Customer fundTransfer(String sourceMobileNo, String targetMobileNo, BigDecimal amount) {
		...
		if (repo.findOne(sourceMobileNo) == null) {
			...
			} else if (tempBalance.subtract(amount).max(minBalance).equals(minBalance)) {
				throw new InsufficientBalanceException("Overdraft limit error.");
			} else {
				Wallet sourceWallet = new Wallet(sourceCustomer.getWallet().getBalance().subtract(amount));
				Wallet targetWallet = new Wallet(targetCustomer.getWallet().getBalance().add(amount));
	...
	public Customer depositAmount(String mobileNo, BigDecimal amount) {
		...
		if (customer != null) {
			...
			Wallet wallet = new Wallet(customer.getWallet().getBalance().add(amount));
	...
	public Customer withdrawAmount(String mobileNo, BigDecimal amount) {
		...
		if (customer != null) {
			...
			if (tempBalance.max(minBalance).equals(new BigDecimal("500"))) {
				...
				Wallet wallet = new Wallet(customer.getWallet().getBalance().subtract(amount));
				customer.setWallet(wallet);
				if (repo.save(customer)) {
					return customer;
				} else {
